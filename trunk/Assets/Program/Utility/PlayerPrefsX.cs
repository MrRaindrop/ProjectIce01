...
namespace Loop
{
    ...
    public class PlayerPrefsX
    {
        ...
        public static bool SetBoolArray(String key, bool[] boolArray)
        {
            ...
            // Make a byte array that's a multiple of 8 in length, plus 5 bytes to store the number of entries
            // as an int32 (+ identifier)
            // We have to store the number of entries, since the boolArray length might not be a multiple of 8,
            // so there could be some padded zeroes
        ...
        public static bool[] GetBoolArray(String key)
        {
            if (PlayerPrefs.HasKey(key))
            {
                ...
                // Make a new bytes array that doesn't include the number of entries + identifier (first 5 bytes)
                // and turn that into a BitArray
                var bytes2 = new byte[bytes.Length - 5];
                System.Array.Copy(bytes, 5, bytes2, 0, bytes2.Length);
                var bits = new BitArray(bytes2);
                // Get the number of entries from the first 4 bytes after the identifier and resize the BitArray to
                // that length, then convert it to a boolean array
        ...
        public static bool SetStringArray(String key, String[] stringArray)
        {
            ...
            // Store the length of each string that's in stringArray, so we can extract the correct
            // strings in GetStringArray
        ...
        private static bool SetValue<T>(
            String key, T array, ArrayType arrayType, int vectorNumber, Action<T, byte[], int> convert) where T : IList
        {
            if (array.Count == 0)
            {
        ...
        private static void GetValue<T>(
            String key, T list, ArrayType arrayType, int vectorNumber, Action<T, byte[]> convert) where T : IList
        {
            if (PlayerPrefs.HasKey(key))
            {
                var bytes = System.Convert.FromBase64String(PlayerPrefs.GetString(key));
